...
export default function Closet({initialAvatar,onSave,coins=2450}){
 ...
 const selectCharacter=item=>setAvatar(a=>({...a,character:item.id,modelUrl:item.modelUrl||'',modelCredit:item.credit||''}));
 const pick=a=>a[Math.floor(Math.random()*a.length)];
 const randomize=()=>{const chosen=pick(characters);setAvatar({...defaultAvatar,character:chosen.id,modelUrl:chosen.modelUrl||'',modelCredit:chosen.credit||'',skin:pick(skinTones),hairColor:pick(hairColors),hairStyle:pick(hairStyles),faceShape:pick(faceShapes),eyeStyle:pick(eyeStyles),brow:pick(brows),facialHair:pick(facialHair),outfit:pick(outfits),accessory:pick(accessories),shoes:pick(shoes),scene:pick(scenes),pose:pick(poses),expressionIndex:Math.floor(Math.random()*6)});};
 const filtered=items=>items.filter(item=>String(typeof item==='string'?item:item.label).toLowerCase().includes(query.toLowerCase()));
 const cards=(items,key,lockedAfter=999,mode='avatar')=><div className="premium-option-grid">{filtered(items).map((item,i)=>{const value=typeof item==='string'?item:item.id,label=typeof item==='string'?item:item.label,locked=i>=lockedAfter;return <button type="button" key={value} disabled={locked} className={`premium-option ${avatar[key]===value?'active':''}`} onClick={()=>key==='character'?selectCharacter(item):update(key,value)}>{locked&&<span className="locked"><Lock size={12}/>80</span>}<span className={`premium-option-preview ${mode==='text'?'text-preview':''}`}>{mode==='avatar'?<AvatarPreview compact avatar={key==='character'?{...avatar,character:value,modelUrl:item.modelUrl||'',modelCredit:item.credit||''}:{...avatar,[key]:value}}/>:<b>{label.slice(0,2).toUpperCase()}</b>}</span><strong>{label}</strong>{key==='character'&&<small>{String(value).includes('female')?'Feminino':String(value).includes('male')?'Masculino':'Personagem'}</small>}</button>})}</div>;
  ...
  <div className="premium-closet-layout">
   ...
   <main className="premium-editor-card">
    ...
    <div className="premium-editor-scroll">
     {tab==='Personagem'&&<><h3>Escolha seu personagem</h3>{cards(characters,'character')}<div className="model-3d-panel"><b>Modelo 3D licenciado</b><p>Use um arquivo GLB/GLTF hospedado com permissão de uso. O CRM mantém o modelo procedural como fallback.</p><input value={avatar.modelUrl||''} onChange={e=>setAvatar(a=>({...a,modelUrl:e.target.value,character:e.target.value?'custom-glb':a.character,modelCredit:e.target.value?'Modelo fornecido pelo administrador':''}))} placeholder="https://.../avatar.glb"/><small>{avatar.modelCredit||'Nenhum modelo externo selecionado.'}</small></div><h3>Tom de pele</h3><div className="premium-swatches">{skinTones.map(c=><button aria-label="Tom de pele" key={c} className={avatar.skin===c?'active':''} style={{background:c}} onClick={()=>update('skin',c)}/>)}</div><h3>Cor do cabelo</h3><div className="premium-swatches hair">{hairColors.map(c=><button aria-label="Cor do cabelo" key={c} className={avatar.hairColor===c?'active':''} style={{background:c}} onClick={()=>update('hairColor',c)}/>)}</div></>}
